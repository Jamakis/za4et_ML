...
bot = telebot.TeleBot('1823672015:AAFLCe6MQw3Uw5vtMDjVU8Q1q8U5n7qDbSI')

@bot.message_handler(commands=['start'])
def msg_start(message):
    bot.send_message(message.chat.id, "Распознавание лиц, без смс и регистрации")
...
for image_path in image_paths:
    ...
    for (x, y, w, h) in faces:
        ...
        if number_actual == number_predicted:
            print(" Правильно {} {}".format(number_actual, conf))
        else:
            print(" Неправильно  {} {}".format(
                number_actual, number_predicted))
        cv2.imshow("Распознавание лица", image[y: y + h, x: x + w])
        cv2.waitKey(1000)
